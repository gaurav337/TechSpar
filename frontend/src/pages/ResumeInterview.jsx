...
const INTERVIEW_STEPS = [
  { title: "introduce yourself", desc: "Introduce yourself based on your resume background and examine your expression and refining abilities" },
  { title: "Project digging", desc: "The interviewer inquires about details of high-profile projects and explores the authenticity of the experience" },
  { title: "Technical questions", desc: "Combined with your technology stack, conduct in-depth inspections to detect technical points and grasp the boundaries." },
  { title: "Summarize feedback", desc: "Issue a clear-dimensional scoring report and suggestions for improvement of weak points" },
];
    ...
    <div className="flex-1 w-full max-w-[800px] mx-auto px-4 py-8 md:px-8 md:py-10 animate-in fade-in duration-500">
      ...
      <div className="mb-10">
        <div className="flex items-center gap-3.5 mb-3">
          ...
          <div className="text-3xl md:text-[34px] font-display font-bold tracking-tight text-text">Resume Mock Interview</div>
        </div>
        <div className="text-[15px] text-dim/90 max-w-[90%] leading-relaxed">
          Upload your resume to start a one-on-one highly customized practical simulation exercise, allowing the AI interviewer to ask critical questions about your experience.
        </div>
      </div>

      {/* Large card for core action area */}
        ...
        <CardContent className="p-6 md:p-8">
          <div className="mb-5 flex items-center justify-between">
            <div className="font-semibold text-text flex items-center gap-2">
              <span className="w-1.5 h-4 bg-primary rounded-full inline-block" /> Candidate asset readiness status
            </div>
            {resumeFile && (
              <Badge variant="outline" className="bg-success/10 border-success/30 text-success font-medium tracking-wider px-3 shadow-sm">
                <CheckCircle2 size={13} className="mr-1.5" /> Resume parsed
              </Badge>
            )}
          </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5 relative z-10">
                  ...
                  <label className={cn("cursor-pointer shrink-0 mt-2 sm:mt-0", uploading && "opacity-50 pointer-events-none")}>
                    <Button variant="outline" size="sm" asChild className="rounded-xl h-10 px-5 font-medium hover:bg-primary hover:text-primary-foreground transition-all hover:border-primary">
                      <span>{uploading ? "Parsing and replacing..." : "Replace with new version..."}</span>
                    </Button>
              ...
              <label className={cn(
                ...
                <div className="text-center">
                  <span className="font-bold text-text text-[16px] block mb-1.5 transition-colors group-hover:text-primary">
                    {uploading ? "Your electronic resume is being read safely..." : "Click or drag your PDF resume here"}
                  </span>
                  <span className="text-[13px] text-dim font-medium px-4 py-1.5 rounded-full bg-hover/80 text-dim">
                    Only supports PDF format · It is recommended that the file does not exceed 20MB
                  </span>
                </div>
                <input type="file" accept=".pdf" className="hidden" onChange={handleUpload} disabled={uploading} />
              </label>
          ...
          <div className="mt-6 pt-5 border-t border-border/50">
            <div className="flex items-center gap-2 mb-2.5">
              ...
              <label className="text-[13px] font-semibold text-text">Target position for this interview</label>
            </div>
            <div className="flex gap-2">
              <Input
                value={targetRole}
                onChange={(e) => setTargetRole(e.target.value)}
                placeholder={targetRoleInferring ? "Inferring from resume..." : "Such as: AI application development engineer / Backend development intern"}
              ...
              <Button
                ...
                title="Re-infer based on resume"
              >
                <Sparkles size={14} className={cn(targetRoleInferring && "animate-spin")} />
              </Button>
            </div>
            <div className="text-[12px] text-dim mt-1.5">The interviewer will adjust the focus and depth of questioning based on the position.</div>
          </div>
          ...
          <div className="mt-6 pt-5 border-t border-border/70 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="text-[13px] text-dim w-full md:w-auto text-center md:text-left">
              Ready to take on the challenge? Click on the right to officially enter the simulation environment
            </div>
            {loading ? (
               <div className="w-full md:w-[220px] rounded-xl bg-card border border-primary/20 py-3.5 px-4 flex items-center justify-center gap-2 relative overflow-hidden shrink-0 shadow-sm">
                 ...
                 <div className="text-[14px] font-semibold text-primary">Build your own match...</div>
               </div>
            ) : (
              <Button
                variant="gradient"
                ...
                onClick={handleStart}
              >
                <Play size={18} className="mr-2 fill-current" /> Start simulation now
              </Button>
            )}
          </div>
        </CardContent>
          ...
          <div className="min-h-[250px]">
            {historyLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full min-h-[250px] px-6 rounded-3xl border-2 border-dashed border-border/60 bg-card/20 text-center">
                ...
                <div className="text-[15px] font-bold text-text mb-2">No training data report yet</div>
                <div className="text-[13px] text-dim max-w-[240px] leading-relaxed">
                  Upload your latest resume above and start the challenge. Your first detailed interview analysis will appear here.
                </div>
              </div>
            ) : (
              <div className="flex flex-col gap-3.5">
                {history.map((s) => {
                  ...
                  const title = reviewed
                    ? "Resume immersive fight"
                    : s.status === "review_failed" ? "The review generation failed, click to try again."
                    : s.status === "reviewing" ? "Review is being generated"
                    : "The interview is not completed, click to continue";
                  return (
                  <Card
                    key={s.session_id}
