      ...
      <Card className="overflow-hidden border-border/80 bg-card/76 mb-6">
        <CardContent className="p-5 md:p-6 xl:p-7">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-dim/80">Interview Copilot</div>
              ...
              <div className="mt-1.5 max-w-2xl text-sm leading-6 text-dim">
                Prepare for interviews in advance and toggle real-time support during the session. Multi-agent architecture predicts interviewer questions and offers real-time response suggestions.
              </div>
            </div>
            <Button variant="gradient" size="lg" className="shrink-0" onClick={onNew}>
              <Plus size={18} /> New Interview Prep
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      {loading ? (
        ...
        <Card className="border-dashed border-border/80 bg-card/55">
          <CardContent className="p-8 text-center">
            ...
            <div className="mt-4 text-lg font-semibold">No Interview Prep Found</div>
            <div className="mt-2 text-sm leading-6 text-dim">
              Click "New Interview Prep", fill in the job description and target company, and Copilot will analyze interviewer strategy for you.
            </div>
            <Button variant="gradient" className="mt-5" onClick={onNew}>
              <Plus size={16} /> New Interview Prep
            </Button>
          </CardContent>
        </Card>
      ) : (
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="font-semibold truncate">
                      {item.company || item.position || "Unnamed"}
                    </span>
                  </div>
                  <Badge
                    variant={
                  ...
                  >
                    {item.status === "done" ? "Ready" : item.status === "running" ? "Preparing" : "Failed"}
                  </Badge>
                </div>
                  ...
                  <div className="flex items-center gap-1">
                    {item.status === "done" && (
                      <Badge variant="outline" className="text-xs group-hover:border-primary/30 group-hover:text-primary transition-colors">
                        <Radio size={10} className="mr-1" /> Ready to Start
                      </Badge>
                    )}
                    {item.status === "running" && (
