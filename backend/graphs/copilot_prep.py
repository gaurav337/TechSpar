"""Copilot Prep Phase — LangGraph Multi-Agent preprocessing pipeline.

Topology: fan-out(Company Researcher, JD Analyst, Fit Analyzer) → fan-in
...
async def _run_fit_analyzer(jd_text: str, user_id: str) -> dict:
    """Agent 3: Resume-to-JD match analysis."""
    resume_context = "Resume not uploaded"
    if _has_resume(user_id):
        try:
            resume_context = str(query_resume(
                "Summarize candidate's project experiences, tech stack, and AI/backend/engineering practices.",
                user_id, top_k=4,
            ))[:5000]
        except Exception as e:
            logger.warning(f"Resume query failed: {e}")
            resume_context = "Resume retrieval failed"
    ...
    resp = await llm.ainvoke([
        SystemMessage(content="You are a match analysis engine. Return ONLY JSON."),
        HumanMessage(content=prompt),
    ])
    try:
        return json.loads(_strip_markdown(resp.content))
...
async def _run_hr_strategy(
    ...
    """Agent 4: Generate questioning strategy tree."""
    ...
    role_title = jd_analysis.get("role_title", "Technical Role")
    ...
    resp = await llm.ainvoke([
        SystemMessage(content="You are an interview strategy engine. Return ONLY JSON."),
        HumanMessage(content=prompt),
    ])
...
async def _run_risk_assessor(
    ...
    resp = await llm.ainvoke([
        SystemMessage(content="You are a risk assessment engine. Return ONLY JSON."),
        HumanMessage(content=prompt),
    ])
    try:
...
async def run_copilot_prep(
    ...
    """Run full Copilot Prep Pipeline.
    ...
    # Bind user for the whole prep pipeline
    set_current_user(user_id)

    profile = get_profile(user_id)

    # Layer 0: Parallel Analysts
    if on_progress:
        await on_progress("Analyzing company information, job requirements, and resume match in parallel...")
    ...
    if on_progress:
        await on_progress("Generating interviewer questioning strategy tree...")
    ...
    if on_progress:
        await on_progress("Assessing risk paths...")

    risk_map, prep_hints, risk_summary = await _run_risk_assessor(strategy_tree, profile, fit_report)

    # Get resume context for storage
    resume_context = ""
    if _has_resume(user_id):
        try:
            resume_context = str(query_resume(
                "Candidate basic information and core experience", user_id, top_k=2,
            ))[:2000]
        except Exception:
            pass
