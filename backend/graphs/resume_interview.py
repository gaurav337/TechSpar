...
def _make_init_interview(user_id: str):
    ...
    async def init_interview(state: ResumeInterviewState) -> dict:
        ...
        resume_ctx = await asyncio.to_thread(
            query_resume, "List all project experience, skills, and educational background of the candidate", user_id
        )
        profile_summary = await asyncio.to_thread(get_profile_summary, user_id)

        target_role = (state.get("target_role") or "").strip() or "Candidate Target Role"
        ...
        response = await llm.ainvoke([
            ...
            HumanMessage(content="The interview has started. Please open the session and ask the candidate to introduce themselves."),
        ])
