...
def _synthesize(question: str, chunks: list[str], user_id: str) -> str:
    ...
    context = "\n\n---\n\n".join(chunks)
    prompt = (
        "You are a resume retrieval assistant. Answer questions based only on the resume snippets below. Do not fabricate information that is not present in the snippets.\n\n"
        f"Resume Snippets:\n{context}\n\nQuestion: {question}\n\nAnswer:"
    )
