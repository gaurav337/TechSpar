...
def _analyze_recording_background(
    ...
    try:
        ...
        if req_recording_mode == "dual":
            ...
            response = llm.invoke([
                SystemMessage(content="You are an interview record analysis engine. Return ONLY JSON, with no other content."),
                HumanMessage(content=structure_prompt),
            ])
            structured = _parse_json_safe(response.content) if "_parse_json_safe" in globals() else _parse_json_response(response.content)
            ...
            eval_response = llm.invoke([
                SystemMessage(content="You are an interview evaluation engine. Return ONLY JSON, with no other content."),
                HumanMessage(content=eval_prompt),
            ])
            eval_result = _parse_json_safe(eval_response.content) if "_parse_json_safe" in globals() else _parse_json_response(eval_response.content)
            ...
            response = llm.invoke([
                SystemMessage(content="You are an audio recording evaluation engine. Return ONLY JSON, with no other content."),
                HumanMessage(content=eval_prompt),
            ])
